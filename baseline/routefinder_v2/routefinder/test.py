...
from experiments.mvmoe.model import MVMoE
from experiments.arc.env import ARCEnv
from experiments.arc.model import ARCModel
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint", type=str, help="Path to the model checkpoint"
    )
    parser.add_argument(
        "--model", type=str, default='arc', help="Path to the model checkpoint"
    )
    parser.add_argument(
        "--leaveout", type=bool, default=False, help="leaveout flag"
    )
    parser.add_argument(
        "--problem",
        type=str,
    ...
    if opts.datasets is not None:
        ...
        data_paths = []
        for fullroot, _, files in os.walk(f"{os.path.abspath(os.path.dirname(__file__))}/data"):
            root = fullroot.split('/data')[-1]
            for file in files:
                if not fullroot.endswith("test"):
                    continue
                if file.endswith(".npz"):
                    ...
                    if str(opts.size) in file:
                        if file == "50.npz" or file == "100.npz":
                            data_paths.append(os.path.join(fullroot, file))
    ...
    if "mvmoe" == opts.model:
        BaseLitModule = MVMoE
    elif "mtpomo" == opts.model:
        BaseLitModule = RouteFinderBase
    elif "moe" == opts.model:
        BaseLitModule = RouteFinderMoE
    elif "routefinder" == opts.model:
        BaseLitModule = RouteFinderBase
    elif "transformer" == opts.model:
        BaseLitModule = RouteFinderBase
    elif 'arc' == opts.model:
        BaseLitModule = ARCModel
    else:
        raise NotImplementedError

    results = {}
    checkpoints = [x for x in opts.checkpoint.split(',')]
    for ckpt in checkpoints:
        ...
        env = MTVRPEnv() if not opts.leaveout else ARCEnv()
        ...
        for dataset in tqdm(data_paths):
            ...
            if results.get(dataset_name, None) is None:
                results[dataset_name] = defaultdict(list)
            results[dataset_name]["cost"].append(-out["max_aug_reward"].mean().item())
            results[dataset_name]["gap"].append(out["gap_to_bks"].mean().item())
            results[dataset_name]["inference_time"].append(inference_time)

        
    print("============================== RESULTS ================================")
        
    for vrp in results.keys():
        output = f"| {vrp:10} | " + " ".join([f"{k}: {sum(results[vrp][k])/len(results[vrp][k]):3.4f} |" for k in ['cost', 'gap', 'inference_time']])
        print(output)
