...
if __name__ == "__main__":
    ...
    testdata_dirname = 'test'
    data_files = []
    for root, dirs, files in os.walk("data"):
        for file in files:
            if file == "50.npz" or file == "100.npz":
                # if file == "100.npz":
                if not root.endswith(testdata_dirname): continue
    ...
    for file in tqdm(data_files, desc="Generating dataset solutions with " + solver):
        ...
        if 'b' in file.split('/')[-3]:
            backhaul_mask = td_test['demand_backhaul'] != 0
            td_test['demand_linehaul'][backhaul_mask] = 0
        if 'm' in file.split('/')[-3]:
            continue
